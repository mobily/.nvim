local Renderer = require("nui-components")
...
M.toggle = function()
  ...
  local process = function()
    local results = fn.kmap(spectre_state.groups, function(group, filename)
      local children = fn.imap(group, function(item)
        local id = tostring(math.random())

        local diff = spectre_utils.get_hl_line_text({
          search_query = h.state.search_query,
          replace_query = h.state.replace_query,
  ...
  local search = function()
    ...
    spectre_state.options["ignore-case"] = not h.state.case_insensitive
    spectre_state.finder_instance = search_engine:new(spectre_state_utils.get_search_engine_config(), search_handler())

    local search_paths = fn.ireject(
      fn.imap(vim.split(h.state.search_paths, ","), function(path)
        return fn.trim(path)
      end),
      function(value)
        return #value == 0
    ...
    local config = {
      cwd = vim.fn.getcwd(),
      search_text = h.state.search_query,
      replace_query = h.state.replace_query,
  ...
  h:on_state_change(function(prev_state, state)
    if #state.search_query < 3 then
      return h:set_state({
        search_results = {},
        search_info = nil,
      })
    ...
    local diff = fn.isome({ "search_query", "case_insensitive", "search_paths" }, function(key)
      return not (prev_state[key] == state[key])
    end)
    ...
    if not (prev_state.replace_query == state.replace_query) then
      process()
    end
  end)

  local body = h.rows(h.columns(
    h.checkbox({
      ...
      on_change = function(is_checked)
        if is_checked then
          local replace_component = h:get_component_by_id("replace_query")
    ...
    h.rows(
      h.columns(
        ...
        h.text_input({
          ...
          flex = 1,
          label = "Search",
          on_change = fn.debounce(function(value, component)
      ...
      h.tree({
        ...
        prepare_node = function(node, line, component)
          ...
          local search_highlight_group =
            component:make_hl(is_replacing and "SpectreSearchOldValue" or "SpectreSearchValue")
